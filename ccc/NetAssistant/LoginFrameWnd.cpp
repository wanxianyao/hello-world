...
void CLoginFrameWnd::Notify(TNotifyUI& msg)
{
	if (msg.sType == _T("click")) 
	{
		...
		else if (msg.pSender->GetName() == _T("SettingBtn"))
		{
			CHorizontalLayoutUI *pHor = static_cast<CHorizontalLayoutUI*>(m_pm.FindControl(_T("urlsetting")));
			if (pHor)
			{
				if (m_bFullwindow)
				{
					this->ResizeClient(288, 198);
					pHor->SetVisible(false);
				}
				else
				{
					this->ResizeClient(288, 263);
					pHor->SetVisible(true);
				}
